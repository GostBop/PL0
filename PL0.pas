...
type
  ...
  newobject = (constant, variable, newprocedure);
...
var
  ...
  table : array [0..txmax] of
          record
            name : alfa;
            case kind : newobject of
              constant : (val : integer);
              variable, newprocedure : (level, adr : integer)
          end;
    ...
    procedure getch ; 
        begin 
            ...
            then begin
                if eof(input) 
                then begin 
                        write(output,'PROGRAM INCOMPLETE'); 
                        close(input);	
                        close(output);
                        exit; 
                    end; 
...
procedure  block(lev, tx : integer; fsys : symset);
    ...
    procedure  enter(k : newobject);
        begin {把newobject填入符号表中}
          tx := tx +1;
          with table[tx] do
          begin  
              name := id;  
  ...
  procedure  listcode;
      ...
      begin  {列出本程序体生成的代码}
          for i := cx0 to cx-1 do
              with code[i] do
              writeln(output, i, mnemonic[f] : 7, l : 3, a : 5)
      end {listcode};
      ...
      begin {statement}
          ...
          begin 
              ...
              if sym <> ident then error(14) 
                  ...
                  i := position(id);
                  if i = 0 then error(111) else
                  with table[i] do
                      if kind = newprocedure then 
...
begin  {主程序}
    ...
    assign(output,paramstr(2));	{将命令行参数str变量赋值给文件变量}
    reset(input);
    rewrite(output);	{打开输入输出文件}
    ...
    else write(output, 'ERRORS IN PL/0 PROGRAM');
    close(input);	
